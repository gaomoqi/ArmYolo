...
#         # 最后一层不应用BatchNorm、ReLU和Dropout
#         x = self.layers[-1](x)
        
#         # 将输出重塑为2x3的雅可比矩阵
#         jacobian = x.view(-1, 2, 3)
...
#     def __init__(self):
#         super(ImageJacobianNet, self).__init__()
#         self.fc1 = nn.Linear(6, 30, bias=False)
...
#         # Split the 60-dimensional output into 6 parts, each with 10 dimensions
#         part1, part2, part3 = torch.split(fc1_output, 10, dim=1)

#         # Extract the 6 components of xdot
...
#         # Split the fc2 weights into 6 parts (each of size corresponding to 10 dimensions)
#         W1, W2, W3 = torch.split(self.fc2.weight, 10, dim=1)
#         J1 = torch.matmul(part1, W1.T)
#         J2 = torch.matmul(part2, W2.T)
#         J3 = torch.matmul(part3, W3.T)

#         J_final = J1 + J2 + J3 
#         # Return the predicted pdot
#         return J_final.view(-1, 4, 1).unsqueeze(2) 
